...
>
> **Claude Code note:** MCP connectors are optional in this version. Local markdown files (CWD, project folder, or Obsidian vault when explicitly pointed at — do NOT scan the whole vault) and pasted content work just as well. If no MCP is connected for a category, fall back to local files + paste. Do not insist on MCP servers.
