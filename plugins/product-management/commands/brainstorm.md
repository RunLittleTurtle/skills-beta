---
description: Brainstormer une idée produit, un problème, ou une question stratégique avec un partenaire de réflexion incisif
argument-hint: "<sujet, problème ou idée à explorer>"
---

# /brainstorm

> If you see unfamiliar placeholders, see [CONNECTORS.md](../CONNECTORS.md). **Claude Code note:** MCP connectors are optional in this version — local markdown files and pasted content work just as well. The skill will not insist on connecting tools.
...
│  + Local files: Pull from markdown notes in your CWD or vault  │
└────────────────────────────────────────────────────────────────┘
```
...
**Claude Code priority order:** local files in CWD → MCP servers if connected → ask user to paste.

If **local markdown files** are available in the CWD (PRDs, research notes, roadmap drafts):
- Offer to read the ones the user points at (do NOT scan the whole filesystem or Obsidian vault without permission)
- Surface relevant content from those files
...
If the user works in Obsidian or a local markdown vault, offer to **save the summary as a markdown file** in the CWD (not in the Obsidian vault unless explicitly permitted).

### 5. Follow Up

After the session, offer:
- "Want me to turn the top idea into a one-pager?" → invokes `write-spec` skill
- "Want me to map this into an opportunity solution tree?"
- "Want me to draft a research plan to test the riskiest assumption?" → invokes `synthesize-research` skill
- "Want me to check how competitors approach this?" → invokes `competitive-brief` skill
